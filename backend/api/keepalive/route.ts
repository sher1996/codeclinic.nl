...
export async function GET() {
  console.log('[keepalive] === BACKEND KEEP-ALIVE REQUEST START ===');
  ...
  try {
    if (!supabase) {
      ...
      return NextResponse.json({ 
        ok: true, 
        message: 'Backend keep-alive endpoint active (Supabase not configured)',
    ...
    return NextResponse.json({ 
      ok: true, 
      message: 'Backend Supabase keep-alive successful',
      timestamp: new Date().toISOString(),
