...
export async function sendAdminApprovalRequest(request) {
  ...
  const subject = `🔐 Admin Access Request - ${request.name}`;
  
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
      <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 30px; border-radius: 10px 10px 0 0; text-align: center;">
        <h1 style="margin: 0; font-size: 24px;">🔐 Admin Access Request</h1>
      ...
      <div style="background: #f8f9fa; padding: 30px; border-radius: 0 0 10px 10px; border: 1px solid #e9ecef;">
        ...
        <div style="background: white; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #007bff;">
          <p style="margin: 0 0 10px 0;"><strong>👤 Name:</strong> ${request.name}</p>
          <p style="margin: 0 0 10px 0;"><strong>📧 Email:</strong> ${request.email}</p>
          <p style="margin: 0 0 10px 0;"><strong>📅 Requested:</strong> ${new Date(request.createdAt).toLocaleString('nl-NL')}</p>
          ${request.reason ? `<p style="margin: 0;"><strong>💬 Reason:</strong> ${request.reason}</p>` : ''}
        </div>
        ...
        <div style="background: #fff3cd; border: 1px solid #ffeaa7; padding: 15px; border-radius: 5px; margin: 20px 0;">
          <p style="margin: 0; color: #856404;"><strong>⚠️ Security Note:</strong> These links are secure and will expire after use. Only click if you recognize this person and want to grant them admin access.</p>
        </div>
...
export async function sendAccessRequestConfirmation(email, name) {
  ...
  const subject = '📧 Admin Access Request Received - CodeClinic';
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
      ...
      <div style="background: #f8f9fa; padding: 30px; border-radius: 0 0 10px 10px; border: 1px solid #e9ecef;">
        ...
        <div style="background: white; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #007bff;">
          <p style="margin: 0 0 15px 0; color: #007bff; font-weight: bold;">✅ Your admin access request has been received!</p>
          ...
          <p style="margin: 0; font-weight: bold;">⏳ Please be patient while we process your request.</p>
        </div>
...
export async function sendBookingConfirmation(booking) {
  ...
  const subject = '📅 Afspraak Bevestigd - CodeClinic';
    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
      ...
      <div style="background: #f8f9fa; padding: 30px; border-radius: 0 0 10px 10px; border: 1px solid #e9ecef;">
        ...
        <div style="background: white; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #28a745;">
          <p style="margin: 0 0 15px 0; color: #28a745; font-weight: bold;">✅ Uw afspraak is succesvol geboekt!</p>
        ...
        <div style="background: white; padding: 20px; border-radius: 8px; margin: 20px 0; border: 1px solid #e9ecef;">
          <h3 style="color: #333; margin-top: 0;">📋 Afspraak Details</h3>
          ...
          <p style="margin: 5px 0;"><strong>Boekingsnummer:</strong> ${booking.id}</p>
          ${booking.notes ? `<p style="margin: 5px 0;"><strong>Notities:</strong> ${booking.notes}</p>` : ''}
        </div>
        
        <div style="background: #fff3cd; border: 1px solid #ffeaa7; padding: 15px; border-radius: 5px; margin: 20px 0;">
          <p style="margin: 0; color: #856404;"><strong>📞 Contact:</strong></p>
        ...
        <div style="background: #d1ecf1; border: 1px solid #bee5eb; padding: 15px; border-radius: 5px; margin: 20px 0;">
          <p style="margin: 0; color: #0c5460;"><strong>ℹ️ Belangrijke informatie:</strong></p>
...
export async function sendUserNotification(email, name, approved) {
  ...
  const subject = approved 
    ? '✅ Admin Access Approved - CodeClinic' 
    : '❌ Admin Access Denied - CodeClinic';
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
      <div style="background: linear-gradient(135deg, ${approved ? '#28a745' : '#dc3545'} 0%, ${approved ? '#20c997' : '#e74c3c'} 100%); color: white; padding: 30px; border-radius: 10px 10px 0 0; text-align: center;">
        <h1 style="margin: 0; font-size: 24px;">${approved ? '✅' : '❌'} Admin Access ${approved ? 'Approved' : 'Denied'}</h1>
        <p style="margin: 10px 0 0 0; opacity: 0.9;">CodeClinic Admin Panel</p>
      </div>
      ...
      <div style="background: #f8f9fa; padding: 30px; border-radius: 0 0 10px 10px; border: 1px solid #e9ecef;">
        ...
        <div style="background: white; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid ${approved ? '#28a745' : '#dc3545'};">
          ${approved ? `
            <p style="margin: 0 0 15px 0; color: #28a745; font-weight: bold;">🎉 Your admin access request has been approved!</p>
          ...
          ` : `
            <p style="margin: 0 0 15px 0; color: #dc3545; font-weight: bold;">😔 Your admin access request has been denied.</p>
